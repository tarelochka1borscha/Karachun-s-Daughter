...
define config.name = _("DaughterTest")
...
define gui.about = _p("""
Разработчик: {a=https://github.com/tarelochka1borscha}tarelochka_borscha{/a}.\n
При разработке использовался искусственный интеллект.\n
Действие происходит в начале XVI века в Великом княжестве Московском в вымышленных поселениях в области истока Северского Донца.
Игра вдохновлена поверьями и традициями этой области, но не претендует на историческую достоверность культуры, быта, одежды и событий того периода.
Также в лоре присутствуют элементы, взятые из западнославянской мифологии (в частности поверья восточной Польши).\n
В истории присутствуют отсылки на народное творчество, возникшее значительно позже указанного исторического периода.
""")
...
define build.name = "DaughterTest"
...
define config.save_directory = "DaughterTest-1721752511"
